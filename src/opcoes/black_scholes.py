"""Module for black scholes formulas implemented with numpy"""
...
def compute_price(pxs, deltas, d_two, K, rf, div, times, call=True):
    ...
    if call:
        return pxs * np.exp(-div * times) * deltas - stats.norm.cdf(d_two) * K * np.exp(-rf * times)
    else:
        return K * np.exp(-rf * times) * stats.norm.cdf(-d_two) + deltas * pxs * np.exp(-div * times)


def compute_gamma(pxs, d_one, vol, div, times):
    # gamma given by: exp(-qT) * n(d1) / (vol * S * sqrt(T))
    return np.exp(-div * times) * stats.norm.pdf(d_one) / (vol * pxs * np.sqrt(times))
...
def compute_greeks(pxs, K, rf, div, sigma, times, call=True, hedge_vols=None, greeks=["delta"]):
